...
- ⚠️ **DEFERRED — L19.** `api-types.ts` is stale (missing `skip_llm_validation`), which is why `api.ts:34-70` hand-patches types with intersections (some redundant). Regenerate and drop the patches. **Not done:** regenerating with openapi-typescript v7 emits default-valued *request* fields (`target_count`, `force_regenerate`, `skip_llm_validation`) as **required** even though the OpenAPI `required` array correctly omits them — which would force those fields on every `generate` call and break the ergonomic callers. Dropping the intersection patches would make it worse, so the stale-types + patches remain the working representation until the generator/config is reconciled.
