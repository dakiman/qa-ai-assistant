...
**Still open — one structural suggestion; structural #2 and the L21 product call are now DONE (L19 was already RESOLVED):**
...
2. **Structural #2 — unit-of-work / commit-once — DONE (2026-07-06, Workstream C).** `get_session` is now a request-scoped unit of work: it commits once when the handler returns and rolls back on any exception. All repositories (`base`, `feature`, `test_case`, `link`, `template`) now `flush()` instead of `commit()`, and the explicit `session.commit()` calls in `generate.py`/`refine.py` were removed (refine flushes before its `refresh`). The `commit=` repo params are kept as accepted-but-ignored no-ops (smaller blast radius). The `link_repository` `IntegrityError` catch was preserved by moving `commit()`→`flush()` *inside* the existing `try`, so duplicate links still map to 409. Verified live via the full write-path smoke matrix (feature/test-case/template CRUD, generate+accept, bulk-status, cascade delete, link create→dup 409→delete, plus a refine round-trip) — every write persisted, duplicate link returned 409 (not 500), and no 500s/tracebacks appeared in the logs.
3. **Product decision (from L21) — DONE:** delete UI is now wired for both test cases (per-card trash button) and features (detail-page Delete button), each behind a confirmation dialog reusing `ui/dialog.tsx`. The `useDeleteTestCase`/`useDeleteFeature` hooks were re-added with real callers. Verified live: test-case delete drops the count (10→9, backend agrees); feature delete redirects to `/features` and the backend returns 404 (H1 cascade intact). *(Done in plan Workstream B.)*
...
- ✅ **RESOLVED — L21.** Dead code hiding missing features: `useUpdateFeature`, `useDeleteFeature`, `useDeleteTestCase`, `featureApi.getStats` + `FeatureStats`, `BulkStatusUpdate`, `TestCaseCard.onDelete`. **Fixed (dead code removed):** `useUpdateFeature` is now genuinely used (`EditFeatureDialog`, M16) and kept; the truly-dead `useDeleteFeature`/`useDeleteTestCase` hooks, `getStats`+`FeatureStats`, the `BulkStatusUpdate` alias, and `TestCaseCard.onDelete` were deleted. **Update (2026-07-06, Workstream B):** delete UI is now wired — `useDeleteTestCase`/`useDeleteFeature` were re-added with real callers behind confirm dialogs (`TestCaseCard` trash button + feature detail-page Delete button). Verified live in the browser (mock provider): test-case delete drops the count and the backend agrees; feature delete redirects to `/features` and the backend returns 404 with the H1 cascade intact.
